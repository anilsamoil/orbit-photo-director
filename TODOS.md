...
- **Pass-detail "visual cues" text (Jack 2026-06-02, deferred from /plan-eng-review)**: short "what to look for" text in the 🌍 expanded thumbnail. **Priority:** P3. **Blocked on content source** — curated targets have `notes`/`caption_hook` but personal/CEO targets do not. Decide source first: (a) Jack hand-enters per-target notes (NASA does this manually in their apps), or (b) an LLM generation pass over coords. Display side is cheap once a source exists. Depends on the v2 personal-target `notes` field (line 66) for the manual path.
- **Pass-detail camera lens/setting suggestion (Jack 2026-06-02, deferred from /plan-eng-review)**: **Priority:** P3. No data today (NASA hand-enters theirs). Two paths: (a) manual per-target field, or (b) a heuristic deriving suggested focal length from `nadir_distance_km` + `angle_off_nadir_deg` (the data exists) → angular size → focal length. Heuristic is feasible but a domain model needing validation against real captures; ship as a labeled estimate if pursued. Evaluate after the encounter/look-angle enhancement soaks.

- **Offline resilience follow-ups (deferred from v1.7.9.0 eng-review NOT-in-scope)**: **Priority:** P3. v1.7.9.0 fixed the two headline offline gaps (generator-vs-offline messaging; black map at world view via z0-3 carto base-tile precache + GIBS TTL 24h→7d). ~~(a) Esri alternate basemap z0-3 precache~~ + ~~(b) GIBS cloud z0-3 precache~~ + night-lights — **SHIPPED v1.7.11.0 (2026-06-08)**: `buildWorldOverlayUrls` precaches z0-3 for Esri imagery, GIBS true-color clouds, and VIIRS Black Marble into dedicated `opd-tiles-esri-base` / `opd-tiles-gibs-base` caches, so the world view of all switchable layers renders offline. Remaining: (c) harden offline detection beyond the probe via an SW-injected `from-cache` header (rejected in eng-review as more complex than the probe — revisit only if the probe proves flaky); (d) deep-zoom / panned-to-area tiles beyond z3 are still online-only by design (precaching every zoom is infeasible).
- ~~**Pre-roll the encounter scan window (fast-follow, raised by v1.7.6.0)**~~ — SHIPPED v1.7.8.0 (2026-06-06). `find_passes` gained `preroll_seconds` (10 min on the two main pass scans); the encounter scan now observes cone crossings that predate the tick, restoring the INITIAL row for imminent passes. Emit filter (`samples[i][0] < window_start → skip`) guarantees no already-past pass is surfaced; launch scanning unchanged. Two orbit tests pin recovery + never-emit-past. Adversarial review (363-pass empirical sweep) found no P1. Daemon-timing: +20 propagations/target/tick, negligible.
- **Minor encounter polish (from v1.7.5.0 adversarial review, low priority)**: **Priority:** P4. (a) 30s sampling means a real encounter 1–29 s before closest can render closest-only on marginal fast passes — interpolation would fix but is bigger scope. (b) ~~`formatLookDirection` shows a spelled side even at near-nadir~~ — addressed in v1.7.7.0: `formatTrackOffset` returns "nadir" (no side) when off-nadir rounds to 0, and the side now comes from the bearing half (the magnitude, which was the unstable part, is no longer shown). Refinement (a) still stands.
- **INITIAL "rising ahead" is hardcoded (from v1.7.7.0 adversarial review)**: **Priority:** P4. The thumbnail INITIAL row hardcodes "rising ahead", correct by the generator invariant that `_find_encounter` only emits fore-of-closest encounters (documented in pass-thumbnail.ts). If a future manifest ever emits a trailing/aft encounter the row would silently mislead; deriving fore/aft from `encounter.rel_bearing_deg` would make it self-correcting. Not wrong today — purely defensive.
...
### V4-P3 — VIIRS night-lights day-side suppression (SHIPPED v1.6.23.0 — 2026-05-29)

Shipped the `maplibregl.addProtocol` luminance-key approach — see `frontend/src/viirs-alpha-protocol.ts`. Dark background pixels (luminance < 30) become transparent with a soft linear ramp through luminance 40, so the layer now paints at 0.95 opacity without darkening basemap/clouds. City lights are no longer muted at zoomed-out views. Ends the v2 → v3.1 → v3.3 → v3.4 → v3.5 opacity-tuning saga.
