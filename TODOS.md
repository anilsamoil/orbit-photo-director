...
- ~~**Pass-detail camera lens/setting suggestion (Jack 2026-06-02)**~~ — **ABSORBED by Photo Conditions Unit 1 (2026-06-11)**: the expanded thumbnail's 📷 camera line shows per-lens ground footprint (computed from `nadir_distance_km` slant geometry) + tracked shutter floors for the real 400/800/1200mm kit, grounded in the Pettit guide instead of a speculative target-size model. The footprint variant sidesteps the "needs validation against real captures" concern — it's pure geometry, validated against the guide's FOV table.
