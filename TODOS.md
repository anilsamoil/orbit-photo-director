...
- **Offline resilience follow-ups (deferred from v1.7.9.0 eng-review NOT-in-scope)**: **Priority:** P3. v1.7.9.0 fixed the two headline offline gaps (generator-vs-offline messaging via a connectivity probe; black map at world view via z0-3 carto base-tile precache + GIBS TTL 24h→7d). Deferred: (a) precache z0-3 world tiles for the **Esri** alternate basemap too (only the default carto basemap is covered today — the Esri toggle is still black offline at world view); (b) precache **GIBS cloud** z0-3 world tiles so the cloud overlay (not just the basemap) renders at world view offline; (c) harden offline detection beyond the probe via an SW-injected `from-cache` header (rejected in eng-review as more complex than the probe — revisit only if the probe proves flaky on slow networks). None blocking; revisit if an operator reports the Esri/cloud offline gap.
