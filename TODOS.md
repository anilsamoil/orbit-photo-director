...
- **Pre-roll the encounter scan window (fast-follow, raised by v1.7.6.0)**: **Priority:** P2. `find_passes` starts its window at `now` (main.py:826, :1262), so a pass whose closest approach lands in the first ~2 min of a tick AND is already inside the off-nadir cone renders closest-only — `_find_encounter` never observes the crossing and returns None (correct, but loses the INITIAL row for the most imminent passes). **v1.7.6.0 widened the threshold 45°→60°, which pushes the crossing further out (~2 min before closest instead of ~1 min), so this now bites passes whose closest is within ~2 min of the tick — a wider sliver than before.** Fix: sample a ~10 min pre-roll before `window_start` for encounter context while still only EMITTING passes with `closest_approach >= now` (must not surface already-past passes). Both Codex review passes endorsed pre-roll. Needs a test that past passes aren't emitted + a daemon-timing check.
