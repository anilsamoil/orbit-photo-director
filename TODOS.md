...
- **Pre-roll the encounter scan window (fast-follow from v1.7.5.0 ship review)**: **Priority:** P2. `find_passes` starts its window at `now` (main.py:826, :1262), so a pass whose closest approach lands in the first ~2 min of a tick AND is already inside 45° off-nadir renders closest-only — `_find_encounter` never observes the crossing and returns None (correct, but loses the "visible now" encounter row for the most imminent passes). Fix: sample a ~10 min pre-roll before `window_start` for encounter context while still only EMITTING passes with `closest_approach >= now` (must not surface already-past passes). Both Codex review passes endorsed pre-roll as the real fix. Needs a test that past passes aren't emitted + a daemon-timing check.
