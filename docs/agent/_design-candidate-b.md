...
`satellite-picker/tle.ts` constructs `SatelliteKey` only after it validates a CelesTrak catalog number or a normalized name key. The MapLibre adapter never sees the raw identifier. No other string assertion is allowed.
...
const impossibleIr: ImageryView = contradictoryIr;
```

The first assignment names a layer outside the generated catalog. The second assignment claims that an IR feed is ready when no satellite covers the view. The intermediate constant proves this does not rely on fresh-object excess-property checking. `tsc --noEmit` rejects both.
...
`effect` is limited to one event-scoped asynchronous request such as `fetchSatelliteTLE`. The runner supplies cancellation and requires the effect to return through `dispatch`. The architecture check rejects DOM access, MapLibre access, event subscription, intervals, and timeouts inside an effect. All long-lived effects remain in `map-core` or the adapter.
...
The adapter installs the imperative effects. It compiles the declared input bindings into a deduplicated handler plan and preserves MapLibre's layer-scoped handlers where their semantics matter. Every native handler emits domain input through `input-router.ts`. `clock.ts` owns the 1 Hz and 30 second intervals. Feature entrypoints declare which domain inputs they consume. They cannot call `map.on`, `addEventListener`, `setInterval`, or `setTimeout`. The architecture check enforces that rule.
...
Each layer declaration carries an install address and a paint address. The install address preserves the current `addLayer` call contract. The paint address owns final z-order.

```ts
export type LayerInstall =
  | Readonly<{
      phase: 'style' | 'load';
      order: number;
    }>
...
export type SourceFamilyDefinition = Readonly<{
  kind: 'source-family';
  idPrefix: 'sat-track-';
  key: 'satellite';
}>;
...
`SourceDefinition` and `LayerDefinition` each include their static variant and the family variant above. `satellite-picker/entrypoint.ts` declares both families once. The reconciler expands them for validated `SatelliteKey` values. A concrete dynamic layer and source still have the generated `LayerId` and `SourceId` template-literal types. No feature can assemble an unchecked runtime id.

The generator emits two deterministic views over the same declarations. The paint catalog sorts by `paintPasses`, then by numeric paint order. The install plan sorts by install phase and install order. It rejects duplicate numeric addresses in either view. The checked paint catalog contains this exact startup sequence:

| Paint order | Layer id | Feature | Install address | Paint address |
| ---: | --- | --- | --- | --- |
| 1 | `esri-imagery-layer` | `basemap-clouds` | `style/10` | `basemap/10` |
| 2 | `carto-dark-layer` | `basemap-clouds` | `style/20` | `basemap/20` |
| 3 | `gibs-clouds-layer` | `basemap-clouds` | `style/30` | `cloud-overlay/10` |
| 4 | `geo-ir-layer` | `ir-overlay` | `style/40` | `cloud-overlay/20` |
| 5 | `ne-coastline-layer` | `basemap-clouds` | `style/50` | `cloud-overlay/30` |
| 6 | `night-lights-global-dim-layer` | `night-lights` | `load/50` | `night-overlay/10` |
| 7 | `terminator-night-fill-layer` | `terminator` | `load/60` | `night-overlay/20` |
| 8 | `viirs-night-lights-layer` | `night-lights` | `load/70` | `night-overlay/30` |
| 9 | `iss-track-layer` | `ground-track` | `load/10` | `track-overlay/10` |
| 10 | `my-targets-casing` | `my-targets-rings` | `load/20` | `target-overlay/10` |
| 11 | `my-targets-layer` | `my-targets-rings` | `load/30` | `target-overlay/20` |
| 12 | `targets-layer` | `targets-pins` | `load/40` | `target-overlay/30` |
| 13 | `terminator-line-layer` | `terminator` | `load/80` | `map-overlay/10` |
| 14 | `subsolar-point-layer` | `terminator` | `load/90` | `map-overlay/20` |
| 15 | `ascent-trajectory-layer` | `launch-corridor` | `load/100` | `map-overlay/30` |
| 16 | `ascent-pad-layer` | `launch-corridor` | `load/110` | `map-overlay/40` |
| 17 | `esri-labels-reference-layer` | `labels` | `load/120` | `label-overlay/10` |

The first five rows compile into the initial style. The remaining twelve reconcile after style load in install-address order. The reconciler ignores entrypoint registration order. It inserts each active layer before the nearest active layer with a higher paint address. It therefore adds the track and target layers first, then makes the three existing `beforeId: 'iss-track-layer'` calls for the night overlays, then adds the remaining overlays and labels. Both the final seventeen-row order and the current twelve-row `addLayer` sequence stay literal contract values.

`lookup-pin-layer`, `dropped-pin-layer`, and the `sat-track-layer-${key}` family use `late-overlay/activation`. That address preserves today's first-materialization order, which can depend on whether a lookup, pin drop, or satellite selection happens first. The generator allowlists those two static ids and that one declared family. A new feature cannot request activation order. Before changing that policy, add a separate behavior decision and update the interaction contract.

Four checks protect order:

1. `generate-map-catalog.ts --check` fails on a duplicate id, an undeclared dynamic family, a missing source, a duplicate install address, a duplicate paint address, an owner mismatch, or generated output drift.
2. `tsc --noEmit` fails when a projection names a `LayerId` or `SourceId` outside the generated union.
3. `map-render-contract.test.ts` mounts the real application against the recording adapter and keeps the current literal seventeen-row expectation. It also keeps the current twelve-row `addLayer` sequence and three `beforeId` values. A second test shuffles registry order and expects all three values to remain identical.
4. `verify-map-pins.mjs` mutates install and paint addresses one at a time. A mutation that leaves the contract test green fails the script.
...
### Slice 7. Cut production over once

Add the generic clocks, input router, persistence, scene reconciler, MapLibre adapter, and recording adapter. Change `main.ts` to load `map/adapter/maplibre/map-app.ts`, pass `MapData`, and dispatch generated external events. Remove the map calls from `updateIssNow`, `loadLookupPane`, `showLaunchOnMap`, and the manifest refresh branch. The map's own clock and event inputs now drive follow, satellite markers, lookup, and launch focus.

Port `map-render-contract.test.ts`, `map-interaction-contract.test.ts`, `map-style-contract.test.ts`, `map-camera-contract.test.ts`, `map-basemap.test.ts`, and `map-overlay-prefs.test.ts` without changing their literal expected behavior. Delete feature-specific setup from `maplibre-double.ts` after the recording adapter replaces it.

Delete `map.ts` in this same commit. Delete `buildStyle`, `renderMap`, `refreshMapForManifest`, `upsertGeoJson`, all twelve `bind*` functions, all bind-once flags, all 55 map module variables, all direct camera calls, and all 25 `map.on` registrations with that file. Delete test-only production setters such as `_setFollowEnvForTest`, `_setCurrentTrackForTest`, `_resetFollowStateForTest`, `_resetScrubTierStateForTest`, and `_resetMapStateForTest` as their tests move through the real facade. Move `viirs-alpha-protocol.ts` into the adapter and delete the old path. Remove the `map.ts` and `main.ts` coverage exclusions.

Add `check-map-architecture.ts` and run it before typecheck, test, and build in this same slice. This cutover is intentionally the largest slice. Splitting the runtime replacement from the caller migration would create the forbidden old and new path. There is never a committed compatibility wrapper at `src/map.ts`, and there is never a second registration route.

### Slice 8. Prove the target rejects drift

Extend `verify-map-pins.mjs` to mutate an install address, a paint address, a source-to-layer link, a default visibility, a camera intent, and a feature directory name. Add architecture-check fixtures that attempt a cross-feature import, a MapLibre import in a feature, a raw `addLayer`, an unregistered layer id, a comment, and a second registry.
...
- The runtime cutover is one larger review unit. Keeping it atomic is the cost of refusing a parallel old and new path.
...
3. The install and paint addresses come from the nearest existing feature with the intended lifecycle and visual relationship.
