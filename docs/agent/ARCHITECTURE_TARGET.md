...
**Slice 0, subtract.** Delete the unused `fetchTLEByCATNR` and `fetchTLEByName` imports and the unused `formatUtc`. Delete the private `wrapLon` in `terminator.ts` and import the one in `iss.ts`. Move `filterPassesByDistance` to a pure `pass-filter.ts`, point both callers at it, and delete `applyDistanceFilter` in `main.ts` along with the comment that falsely claims it delegates. Delete `refreshMapForManifest`, `_resetViirsFallbackForTest` and `reflectCloudsButtonState`, all of which have no live caller or are copies. Delete the stale eleven-line header at the top of `map.ts`. Turn on `noUnusedLocals` and fix the 12. Rewrite the remaining source-text assertions in `map-ir.test.ts` and `map-night-lights.test.ts` as runtime assertions, so no test reads `map.ts` as a string.
