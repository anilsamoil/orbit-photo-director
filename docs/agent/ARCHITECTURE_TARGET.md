...
The shape the map should have. `ARCHITECTURE_NOW.md` describes what existed at the start; this describes what replaces it and in what order. Slices 0 to 16 have shipped; each slice's record under "Phase 3 slices" says what it actually did.
...
**Slices 12 to 16, wave three.** Shipped together. `features/iss-marker/`, `features/targets/`, `features/launch-corridor/`, `features/time-scrub/`, and `features/follow-iss/` own those capabilities. `PREF_KEYS` gained `bearingMode`. The composition root calls each feature at the historical `renderMap` site, so the layer sequence and the handler list did not move. One `styledata` listener still covers launch mode and the target pins; the launch feature reaches the target feature through `bindLaunchTargets`, and `focusLaunchOnMap` reaches follow through `bindExitFollow`. The scrub reaches the marker, the camera, and the forecast deferral through `bindScrubServices`. The 1 Hz tick stays in the root: snap, then marker, then bearing. `getSatelliteTopbarReadouts` moved into `features/satellites/` and is re-exported. `src/map.ts` is deleted. `src/map/index.ts` is the composition root, 482 lines, and its one module-level `let` is `core`. The vendor and adapter exceptions for `map.ts` are gone. Typecheck clean, 1889 tests green, lever 16 of 16.
