...
Next to `launches.json`, `.json.fetch.lock` serializes attempts without waiting and `.json.request.json` records the current attempt plus at most 24 earlier attempts. Only status, timestamps, exception class and selected non-sensitive cache headers are recorded. Do not delete these files to bypass a cooldown. Missing state is initialized before requesting; corrupt state gets one bounded forensic copy and a durable two-hour recovery wait. Unwritable state refuses network requests. Keep this directory persistent, with fetcher and publisher under the same user.

Attempts are reserved before the GET with a 15-minute minimum interval, including after process loss. Errors back off 15, 30, 60, then at most 120 minutes. On 429/503, valid server `Retry-After` values through seven days are honored, including HTTP dates; implausibly longer values are recorded as rejected and trigger a conservative two-hour wait. Cooldown, TTL and successful-fetch timestamps use wall clock, not the generator's older tick-start timestamp. There is no inline retry, sleep or redirect follow. Success resets the failure count. The upstream quota failure's historical cause is not claimed fixed; these receipts make a recurrence diagnosable.

A pending source receipt is prepared before replacing the cache body. Disk-full failures during preparation preserve the old pair. An interruption after body replacement is repaired under the fetch lock before the next TTL check, without refetching. A publisher racing the two renames still fails closed on a hash mismatch and retries at its next normal check; no unverified pair is published.
