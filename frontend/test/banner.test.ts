...
describe('bannerFromManifest', () => {
  ...
  it('green when fresh and freshness.ok', () => {
    const result = bannerFromManifest('2024-10-17T11:55:00Z', true, now);
    expect(result.level).toBe('green');
    expect(result.text).toContain('Last updated');
  });

  it('yellow when 40-60 min old and ok', () => {
    const result = bannerFromManifest('2024-10-17T11:15:00Z', true, now);
    expect(result.level).toBe('yellow');
    expect(result.text).toContain('running slow');
  });

  it('red when > 60 min old', () => {
    const result = bannerFromManifest('2024-10-17T10:30:00Z', true, now);
