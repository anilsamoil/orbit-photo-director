...
describe('Cloudflare Access re-auth escape hatch', () => {
  it('the reauth path is denylisted in the SW navigation route', async () => {
    ...
    const denylisted = [/^\/api\//, /^\/v\//, /\/manifest\.json/];
    expect(denylisted.some((rx) => rx.test(ACCESS_REAUTH_PATH))).toBe(true);
    // And the SW config must still declare that denylist.
    expect(await swCfg()).toMatch(/navigateFallbackDenylist:\s*\[/);
    expect(await swCfg()).toMatch(/\/\^\\\/api\\\//);
  });
